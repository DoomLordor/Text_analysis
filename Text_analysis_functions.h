...
void delete_stop_word(vector <string>& array_to_sort, vector <int>& frequency, string file_way); // удаление стоп слов которые наход€тс€ в соответствующем словаре
...
void lemmatizator(vector <string>& array_to_sort); // лемматизаци€ слов
...
// работа с преподавател€ми
void teacher_position_number(vector <int>& frequency, vector <int>& numbers); //определение позиций преподвателей
void teacher_input(vector <fio>& input_array, string file_way); // ввод ‘»ќ преподавателей их числа их публикаций
void teacher_keyword(vector <fio>& input_data, vector <string>& input_array_word, vector <int>& frequency, vector <int>& numbers); // добавление преподавател€м их ключевых слов
void sort_array(vector<fio>& sort_array); // преподавателей по фамили€м
