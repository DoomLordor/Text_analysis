 ...
 namespace Solarix
 {
  ...
  class KB_Facts : lem::NonCopyable
  {
   public:
    ...
    int query_mode; // 0 - загружать все в пам€ть и кэшировать, 1 - все аргументы по word, 2 - все аргументы по id_entry
    bool check_features; // true если даже дл€ query_mode!=0 необходимо после проверки лемм/лексем проверить еще и координатны пары...

    int violation_score; // штраф за нарушение
    PredicateTemplate *violation_handler; // шаблон предиката, который будет создан в случае нарушени€ данного факта

    #if defined SOL_LOADTXT && defined SOL_COMPILER
    bool allow_generic; // если при компил€ции не нужно выбрасывать ошибку в случае, если очередной факт имеет не-индексируемый способ доступа
    #endif
  ...
  class KB_Argument : lem::NonCopyable
  {
   public:
    ...
    boost::wregex rx; // дл€ функции @regex/@regex_strict
  ...
  class KB_Fact : lem::NonCopyable
  {
   public:
    ...
    int bool_return; // возвращаемый результат дл€ булевской проверки
    int false_score; // опционально - если дл€ bool результата (обычно false) возвращаетс€ еще штраф. «начение 0 - значит штраф неприменим, и выполн€етс€ жесткий отказ.
