...
// #include <lem/solarix/type_field.h>
 ...
 namespace Solarix
 {
  ...
  {
   uint8_t res=0;
   for( lem::Container::size_type i=0; i<a.size(); i++ )
    res ^= ( a[i].GetCoord().GetIndex() + a[i].GetState()  );
 ...
 class SG_KeyHandler
 {
  ...
  public:
   SG_KeyHandler(void) { key=index=UNKNOWN; }
 ...
 class Link_Generator
 {
  ...
  public:
   ...
   bool IsMultitier(void) const { return multitier; }
   ...
   inline Tree_Link getReverseLink(void) const { return reverse_link; }
 };
 #endif
...
*/
   ...
   SynGram( const SynGram& );
   void operator=( const SynGram& );

   const GraphGram& GetGraphGram(void) const;

   void PrintMap_2( OFormatter &txtfile, int iclass );

   virtual GramClass* Create_Class(void) const;

  public:
   SynGram( Dictionary & dict, const Dictionary_Config &cfg );
   virtual ~SynGram(void);
   ...
   int GetMaxMLLen(void) const;

   virtual const lem::UCString GetName(void) const;
   ...
   inline Languages& languages(void) const { return *langs; }

   inline const SG_Net& Get_Net(void) const { return *net; }
   inline       SG_Net& Get_Net(void)       { return *net; }
   ...
   WordEntries& GetEntries(void) { return *word_entries; }
   void SetEntries( WordEntries *x );

   AuxFormTypes& GetAuxFormTypes(void) { return *aux_form_types; }
